...
resource "google_compute_address" "master_lb" {
  name         = "${var.prefix}-master-lb-address"
  address_type = "EXTERNAL"
  region       = var.region
}

resource "google_compute_forwarding_rule" "master_http_lb" {
  name = "${var.prefix}-master-http-lb-forward-rule"

  ip_address = google_compute_address.master_lb.address
  port_range = "80"

  target = google_compute_target_pool.master_lb.id
}

resource "google_compute_forwarding_rule" "master_https_lb" {
  name = "${var.prefix}-master-https-lb-forward-rule"
